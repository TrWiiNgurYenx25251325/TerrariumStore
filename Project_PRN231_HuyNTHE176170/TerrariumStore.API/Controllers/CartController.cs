...
namespace TerrariumStore.API.Controllers
{
    ...
    public class CartController : ControllerBase
    {
        ...
        public async Task<IActionResult> CheckoutCartWithAddress(CheckoutDTO checkoutDto)
        {
            ...
            if (string.IsNullOrEmpty(checkoutDto.RecipientName))
                return BadRequest("Vui lòng cung cấp tên người nhận.");
                
            if (string.IsNullOrEmpty(checkoutDto.RecipientPhone))
                return BadRequest("Vui lòng cung cấp số điện thoại người nhận.");
            ...
            var order = new Order
            {
                ...
                RecipientPhone = checkoutDto.RecipientPhone,
        ...
        public async Task<IActionResult> CheckoutCart(int userId)
        {
            ...
            // Kiểm tra nếu người dùng không có số điện thoại
            if (string.IsNullOrEmpty(user.Phone))
                return BadRequest("Vui lòng cập nhật số điện thoại trong thông tin cá nhân trước khi thanh toán.");
            ...
            var order = new Order
            {
                ...
                RecipientName = user.FullName, // Sử dụng tên người dùng
                RecipientPhone = user.Phone, // Sử dụng số điện thoại người dùng
                OrderDetails = cart.CartItems.Select(ci => new OrderDetail
                {
                    ProductId = ci.ProductId,
                    Quantity = ci.Quantity,
